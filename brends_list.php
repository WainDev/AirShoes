...
<body>
    <div class="container">
        ...
        $brand_id = $_GET['id']; // Get the brand ID from the GET request
        $query = "SELECT * FROM Brands WHERE Id = '$brand_id'"; // Modify the query to filter by brand ID
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $brand = $stmt->fetch(PDO::FETCH_ASSOC);
        $brand_name = $brand['NameBrands']; // Get the brand name from the database
        ?>
        <div class="header_block mt-4 p-4 d-flex fl align-items-center justify-content-between">
            <h1>Каталог/<?php echo $brand_name; ?></h1>
            <a class="pay_button" href="/index.php">Назад</a>
        </div>
        <div class="card_blocks d-flex align-items-center justify-content-around">
            <?php
            if ($conn) {
                $brand_id = $_GET['id']; // Get the brand ID from the GET request
                $query = "SELECT * FROM Products WHERE Brands_Id = '$brand_id'"; // Modify the query to filter by brand ID
                $stmt = $conn->prepare($query);
                $stmt->execute();
            ...
            ?>
                    ...
                    <div class="d-flex fl align-items-center justify-content-between">
                        <div class="card_product  card shadow mt-5 p-3 mb-5 bg-body-tertiary rounded" style="width: 18rem">

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!class_exists('Database')) {
        ...
        if (!$db->getConnection()) {
            ...
            if (!empty($_POST['brandname']) && !empty($_POST['brandurl']) && !empty($_POST['priority'])) {

                $stmt = $conn->prepare("INSERT INTO `Brands` (`Id`, `NameBrands`, `ImgUrl`, `Priority`) VALUES (NULL, :brandname, :brandurl, :priority)");


                $stmt->bindParam(':brandname', $_POST['brandname']);
                $stmt->bindParam(':brandurl', $_POST['brandurl']);
    ...
    <div class="container">
        <div class="admins_list mt-5 shadow-lg p-3 mb-5 bg-body rounded">
            <div class="header_form d-flex align-items-center justify-content-between p-2">
                <h1>Добавление бренда</h1> <a href="/AdminPanel/index.php"><img src="../../../Assets/image/close.svg" alt=""></a>
            </div>
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="admins_form">
                <input class="admin_input" type="text" name="brandname" placeholder="Имя бренда" />
                <input class="admin_input" type="text" name="brandurl" placeholder="Картпинка бренда" />
